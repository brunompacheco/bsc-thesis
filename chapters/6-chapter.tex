...
\begin{figure}[h]
    ...
    \caption{Learning curve of the models trained with fewer states.}
...
\begin{figure}[h] % TODO
    \centering
    \includegraphics[width=0.8\textwidth]{images/exp_4_iae.pdf}
    \caption{Learning curve for \gls{PIDEQ}s with different $\kappa$ values. Only one model was trained with $\kappa=0$ because the training took over 30 times longer.}
...
Models using Broyden's method achieve a smaller error, but the iterations took much longer, with an average value of XXX ms in comparison to YYY ms using Anderson Acceleration.  % TODO compute times
...
In comparison to the baseline, the previous experiments showed how a much smaller model can be used and with the simple iteration method, which provides a faster training.
Figure XXX shows the performance of this final model in comparison to the baselines.
In comparison to the initial \gls{PIDEQ}, there was a significant decrease in the error but the biggest impact was in training time, which decreased from XXX ms to YYY ms for epoch on average. 
...
The performance of the final model and the small \gls{PINN} model can be assessed in figure \ref{fig:final-iae}, in comparison to the baseline models. The final models can be assessed visually, in comparison to \gls{RK4}, through the graphics in figure \ref{fig:final-vdp}.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/final_iae.pdf}
    \caption{Learning curve of the final models in comparison to the baselines. "Final PINN" are the small \gls{PINN} models, with only 52 parameters. "Final PIDEQ" are the \gls{PIDEQ} models with 5 states and using the simple iteration method as solver for the forward pass.}
    \label{fig:final-iae}
\end{figure}
...
Both final models take around the same number of epochs to converge, but the \gls{PINN} model is able to achieve much smaller error, almost as low as the baseline model.
The \gls{PINN} models, though, have a smaller training time.
