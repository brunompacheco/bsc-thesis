...
In this model, each component is a simple affine operator followed by a nonlinear \textit{activation} function, i.e., following the notation previously presented, we can define a deep feedforward neural network with $L$ layers as a \textit{parametrized} function $f_{\gls{param}}$ such that, for a given input $\bm{x}$,
\begin{align*}
    \bm{z}^{[0]} &= \bm{x} \\
...
Applied to the equations above, the first operation would be to compute $\bm{u}^T\gets\nabla_{f_{\gls{param}}} l$, which would be used in the vector-Jacobian product $\bm{u}^T \frac{d f_{\gls{param}}}{d \gls{param}^L}$ to compute the gradient with respect to $\gls{param}^{L}$.
Then, the intermediate result can be updated through the vector-Jacobian product $\bm{u}^T\gets \bm{u}^T \frac{d f^L_{\gls{param}^L}}{d f^{L-1}_{\gls{param}^{L-1}}}$, which is useful to compute the gradient with respect to $\gls{param}^{L-1}$.
