...
\footnotetext{Each iteration of RK4 makes 5 $\mathcal{N}$ function evaluations while Euler's method's makes only one evaluation for iteration, so we can estimate that RK4 is 5 times more expensive than Euler's.}
...
\footnotetext{Loosely defined, a limit cycle can be seen as the image of the solution when it becomes periodic. Refer to textcite[REFTO Grimshaw, 1990] for a more precise definition.}
